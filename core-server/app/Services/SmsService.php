    ...
    public function sendSms(string $number, string $message): array
    {
        try {
            ...
            if ($curlError) {
                return [
                    'success' => false,
                    'message' => 'SMS sending failed (cURL Error)',
                    'error'   => $curlError
                ];
            }
    
            // ✅ API returns plain text. Normalize it.
            $responseText = trim($response);
    
            // ✅ bulksmsbd success response looks like: "SMS SUBMITTED"
            if (str_contains(strtoupper($responseText), 'SUCCESS') || str_contains(strtoupper($responseText), 'SUBMITTED')) {
                return [
                    'success' => true,
            ...
            return [
                'success' => false,
                'message' => "SMS sending failed: {$responseText}",
                'response' => $responseText
            ];
